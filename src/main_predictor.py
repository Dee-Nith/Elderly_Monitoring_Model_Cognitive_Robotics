...
# === SYSTEM CONFIG ===
log_every = LOG_EVERY_N_FRAMES
logger = ActionLogger(filename=LOG_FILE)
...
def trigger_alert(predicted_action):
    if predicted_action in HIGH_PRIORITY_ACTIONS:
        print(f"[ALERT] High-priority action detected: {predicted_action}")
        return True
    return False

def main():
    """Main function to run the elderly monitoring system"""
    trajectory_history = []
    frame_count = 0
    ...
    while cap.isOpened():
        ...
        color = ALERT_COLOR if alert_flag else NORMAL_COLOR
        ...
        cv2.putText(frame, f'Action: {predicted_action}', (10, 120),
                    cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE + 0.1, color, FONT_THICKNESS)

        if alert_flag:
            cv2.putText(frame, "⚠️ ALERT", (10, 160),
                        cv2.FONT_HERSHEY_SIMPLEX, ALERT_FONT_SCALE, ALERT_COLOR, ALERT_FONT_THICKNESS)
    ...
    print("Elderly Monitoring System stopped.")
